...
keep_prob = tf.placeholder(tf.float32) # dropout的百分比
xs = tf.placeholder(tf.float32,[None,64]) # 不规定多少个sample但是有64个属性
...
for i in range(1000):
    sess.run(train_step,feed_dict={xs:X_train,ys:y_train,keep_prob:0.5}) # 每次保留0.5。0.6就是保留0.6
