...
def compute_accuracy(v_xs, v_ys):
    ...
    correct_prediction = tf.equal(tf.argmax(y_pre,1), tf.argmax(v_ys,1)) # 比较概率最大的坐标
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32)) # 求平均
...
x_image = tf.reshape(xs,[-1,28,28,1]) # 处理一下传入图片的信息-1就是不管由多少sample，1的颜色(黑白的)
